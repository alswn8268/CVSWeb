...
<%@ include file="../util/hfer/header.jsp" %>
	...
	<div align="center">
		<input type="text" class="form-control" id="itemName" onkeyup="fSearch()" placeholder="검색할 내용을 입력하세요." style="display: inline !important; width: 800px;">
		<button type="button" class="btn btn-primary" onclick="searchFunction()" style="display: inline !important">검색</button>
		<div id="dropdown-menu" class="ddm"></div>		
	</div>
		...
		<c:if test="${searchList != null}">
			...
			<c:forEach var="s_vo" items="${slist}" varStatus="i">
				<span style=" padding: 5px; vertical-align: middle;">
					<a style="font-size: 16px; padding: 3px 10px;" href="itemListSort?itemName=${s_vo.keyword}&currentPage=${itemList.currentPage}&mode=11">${s_vo.keyword}&nbsp;</a>	
				</span>
			</c:forEach>
		</c:if>
	</div>
